...
import { createAuthorRequestSchema, updateAuthorRequestSchema, validate } from "@/lib/validation";
...
async function POST(request: NextRequest, response: NextResponse) {
  try {
    ...
    const createdAuthors = await db.insert(author).values(authorData).returning();
    
    if (!createdAuthors || !Array.isArray(createdAuthors) || createdAuthors.length === 0) {
...
async function PATCH(request: NextRequest, response: NextResponse) {
  ...
  try {
    ...
    if (data.socials && data.socials.length > 0) {
      ...
      const existingSocialIds = existingAuthor.socials.map(social => social.id);
      
      for (const social of data.socials) {
        if (social.id) {
          // Update existing social
          if (existingSocialIds.includes(social.id)) {
            await db.update(authorSocials)
              .set({
                type: social.type,
                url: social.url,
                updatedAt: now
...
async function DELETE(request: NextRequest, response: NextResponse) {
  ...
  try {
    ...
    if (existingAuthor.titles && existingAuthor.titles.length > 0) {
      return NextResponse.json({ 
        error: "Cannot delete author with associated titles. Remove titles first." 
      }, { status: 400 });
    }
