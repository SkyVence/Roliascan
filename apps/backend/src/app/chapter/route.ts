...
import { createChapterRequestSchema, updateChapterRequestSchema, validate } from "@/lib/validation";
    ...
    if (chapterId) {
      ...
      try {
        ...
        try {
          ...
          console.log("Chapters query succeeded, got", chapters.length, "results");
        } catch (chaptersError) {
          console.error("Error fetching chapters:", chaptersError);
          // Fallback to a simpler query without relations if the main query fails
          try {
            const simpleResults = await db.select().from(chapter).limit(limit).offset(offset);
            chapters = simpleResults || [];
            console.log("Simple fallback query succeeded, got", chapters.length, "results");
          } catch (fallbackError) {
...
async function POST(request: NextRequest, response: NextResponse) {
  try {
    ...
    const createdChapters = await db.insert(chapter).values(chapterData).returning();
    
    if (!createdChapters || !Array.isArray(createdChapters) || createdChapters.length === 0) {
...
async function PATCH(request: NextRequest, response: NextResponse) {
  ...
  try {
    ...
    if (data.chapterNumber !== undefined) updates.chapterNumber = data.chapterNumber;
    ...
    if (data.content && data.content.length > 0) {
      ...
      const existingContentIds = existingChapter.content.map(content => content.id);
    ...
    if (data.removeContent && data.removeContent.length > 0) {
      await db.delete(chapterContent)
        .where(
          and(
